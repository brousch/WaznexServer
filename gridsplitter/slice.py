...
def TransformDots(dots, searchImage, originalImage):
    ...
    def TransformDot(dot):
        return (int(dot[1] * originalImage.size[0] / searchImage.size[0]), int(dot[2] * originalImage.size[1] / searchImage.size[1]))
...
def SliceSquares(imageOriginal, channel, drawDebuggingGrid = None):
    ...
    if(len(dots) > 0):
        ...
        if(len(dots) < len(topColumnDots) * len(leftRowDots)):
            ...
            topVec = (dots[topColumnDots[len(topColumnDots) - 1]][0] - dots[topColumnDots[0]][0], dots[topColumnDots[len(topColumnDots) - 1]][1] - dots[topColumnDots[0]][1])
            topFullLength = hypot(topVec[0], topVec[1])
            rightVec = (dots[rightRowDots[len(rightRowDots) - 1]][0] - dots[rightRowDots[0]][0], dots[rightRowDots[len(rightRowDots) - 1]][1] - dots[rightRowDots[0]][1])
            rightFullLength = hypot(rightVec[0], rightVec[1])
            bottomVec = (dots[bottomColumnDots[len(bottomColumnDots) - 1]][0] - dots[bottomColumnDots[0]][0], dots[bottomColumnDots[len(bottomColumnDots) - 1]][1] - dots[bottomColumnDots[0]][1])
            bottomFullLength = hypot(bottomVec[0], bottomVec[1])
            leftVec = (dots[leftRowDots[len(leftRowDots) - 1]][0] - dots[leftRowDots[0]][0], dots[leftRowDots[len(leftRowDots) - 1]][1] - dots[leftRowDots[0]][1])
            ...
            if(len(topColumnDots) > len(bottomColumnDots)):
                ...
                for c in range(1, len(topColumnDots) - 1):
                    topLength = hypot(dots[topColumnDots[c]][0] - dots[topColumnDots[0]][0], dots[topColumnDots[c]][1] - dots[topColumnDots[0]][1])
                    topRatio = topLength / topFullLength
                    newDot = (dots[bottomColumnDots[0]][0] + bottomVec[0] * topRatio, dots[bottomColumnDots[0]][1] + bottomVec[1] * topRatio)
                    dots.append(newDot)
                    if(drawDebuggingGrid):
                        drawDebuggingGrid.ellipse((newDot[0] - 10, newDot[1] - 10, newDot[0] + 10, newDot[1] + 10), fill="blue")
            ...
            if(len(leftRowDots) > len(rightRowDots)):
                ...
                for r in range(1, len(leftRowDots) - 1):
                    leftLength = hypot(dots[leftRowDots[r]][0] - dots[leftRowDots[0]][0], dots[leftRowDots[r]][1] - dots[leftRowDots[0]][1])
                    leftRatio = leftLength / leftFullLength
                    newDot = (dots[rightRowDots[0]][0] + rightVec[0] * leftRatio, dots[rightRowDots[0]][1] + rightVec[1] * leftRatio)
                    dots.append(newDot)
                    if(drawDebuggingGrid):
                        drawDebuggingGrid.ellipse((newDot[0] - 10, newDot[1] - 10, newDot[0] + 10, newDot[1] + 10), fill="blue")
            ...
            for row, leftDot in enumerate(leftRowDots[1:-1]):
                ...
                for col, topDot in enumerate(topColumnDots[1:-1]):
                    ...
                    topLen = hypot(dots[topDot][0] - dots[topColumnDots[0]][0], dots[topDot][1] - dots[topColumnDots[0]][1])
                    topRatio = topLen / topFullLength
                    bottomLen = hypot(dots[bottomDot][0] - dots[bottomColumnDots[0]][0], dots[bottomDot][1] - dots[bottomColumnDots[0]][1])
                    ...
                    newDot = (dots[leftDot][0] + rowVec[0] * ratioWithinRow, dots[leftDot][1] + rowVec[1] * ratioWithinRow)
                    dots.append(newDot)
                    if(drawDebuggingGrid):
                        drawDebuggingGrid.ellipse((newDot[0] - 10, newDot[1] - 10, newDot[0] + 10, newDot[1] + 10), fill="blue")
